...
export const PostCareers = () => {
  ...
  const [templateField, setTemplateField] = React.useState('careers');
  const [parentpageField, setParentPageField] = React.useState([]);
  const [jobCategories, setJobCategories] = React.useState([]);
  const [skillsCategories, setSkillsCategories] = React.useState([]);
  const [experienceCategories, setExperienceCategories] = React.useState([]);
  const [locationCategories, setLocationCategories] = React.useState([]);
  ...
  React.useEffect(() => {
    const fetchPage = async () => {
      ...
      try {
        const res = await fetch(`/api/careers/get/${postid}`, {
          method: 'GET',
          headers: {
        ...
        setCustomMetaFields(data?.customMetaFields);
        setJobCategories(data?.categories?.job);
        setSkillsCategories(data?.categories?.skills);
        setExperienceCategories(data?.categories?.experience);
        setLocationCategories(data?.categories?.location);
  ...
  const handleSubmit = async (e) => {
    ...
    if (postid || actionType === 'edit') {
      ...
      try {
        const res = await fetch(`/api/careers/update/${postid}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      // CREATE POST
      try {
        const res = await fetch('/api/careers/create', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (data.success) {
          ...
          setActionType('edit');
          navigate(`/dashboard/new-career?page=${randomPageId}&action=edit`);
        }
      } catch (error) {
        toast.error(error.message);
      }
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div>
          ...
          <CategorySelector
            pageType={parentpageField}
            type="skills"
            selected={skillsCategories}
            onChange={setSkillsCategories}
          />
          <CategorySelector
            pageType={parentpageField}
            type="experience"
            selected={experienceCategories}
            onChange={setExperienceCategories}
          />
          <CategorySelector
            pageType={parentpageField}
            type="location"
            selected={locationCategories}
            onChange={setLocationCategories}
          />
        </div>
      </div>
    </form>
